        ...
        for (int j = 0; j < line.size(); j++) {
            if (isdigit(line[j])) {
                ...
                if (temp.size()) {
                    ...
                    int numStart = j - temp.size();
                    int xbegin = max(0, numStart - 1);
                    int xend = min(j+1, cols);
                    int ybegin = max(0, i - 1);
                    int yend = min(rows, i + 2);

                    for (int y = ybegin; y < yend; y++) {
                        for (int x = xbegin; x < xend; x++) {
                            if (lines[y][x] == '*') {
                                mmap[y * rows + x].push_back(stoi(temp));
                            }
                        }
                    }
